#!/usr/bin/env node
/**
 * 네이버 즐겨찾기 → stores.json 동기화 스크립트
 * 사용법: node scripts/sync.js
 *
 * 1. 네이버 즐겨찾기 폴더에서 북마크 목록 가져오기
 * 2. 새로 추가된 매장 감지
 * 3. 상세 정보 수집 (이미지, 메뉴, 인스타, 리뷰 등)
 * 4. data/stores.json 업데이트
 */
...
function getBusanSubRegion(address, district) {
  if (district === '부산진구') return JEONPO_ROADS.some(r => address.includes(r)) ? '전포' : '서면';
  if (district === '수영구') return (address.includes('광안') || address.includes('남천')) ? '광안리' : '수영';
  const map = { '해운대구':'해운대','연제구':'연제구','금정구':'금정구','기장군':'기장','영도구':'영도','동래구':'동래','중구':'남포/중구','동구':'동구','강서구':'강서구','북구':'북구','사하구':'사하구','남구':'남구','사상구':'사상구' };
  return map[district] || district;
}

function classifyCategory(naverCategory) {
  if (!naverCategory) return '카페';
  const cat = naverCategory.toLowerCase();
  if (cat.includes('베이커리') || cat.includes('제과') || cat.includes('빵')) return '베이커리';
  if (cat.includes('브런치')) return '브런치';
...
async function collectStoreData(sid, bookmark) {
  ...
  // Visitor reviews from review page
  let visitorReviews = [];
  try {
...
async function main() {
  ...
  for (const folder of FOLDERS) {
    ...
    if (data?.bookmarkList) {
      console.log(`${folder.name}: ${data.bookmarkList.length}개`);
      allBookmarks.push(...data.bookmarkList.map(b => ({ ...b, folderName: folder.name })));
    } else {
      console.error(`⚠ 폴더 ${folder.name} 로드 실패!`);
      fetchFailed = true;
    }
  }
  ...
  // Update hours for existing stores (quick refresh via Summary API)
  ...
  // Merge: keep existing + add new + remove deleted
  const finalStores = [
    ...kept,
    ...newStores,
  ];
